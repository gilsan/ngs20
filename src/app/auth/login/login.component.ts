...
export class LoginComponent implements OnInit, OnDestroy {
      ...
      this.subs.sink = this.authService.loginDiag(this.loginGroup.value.user, this.loginGroup.value.passwd)
        .subscribe((data) => {
          ...
          if (data.message === 'WRONGID') {
            ...
            localStorage.setItem('diaguser', JSON.stringify({ userid: this.loginGroup.value.user, pw: this.loginGroup.value.passwd }));
      ...
      this.subs.sink = this.authService.loginPath(this.loginGroup.value.user, this.loginGroup.value.passwd)
        .subscribe((data) => {
          ...
          if (data.message === 'WRONGID') {
            ...
            localStorage.setItem('pathuser', JSON.stringify({ userid: this.loginGroup.value.user, pw: this.loginGroup.value.passwd }));
  ...
  register(): void {
    ...
    window.open(url, "사용자등록", "top=100, left=200, width=450, height=535, toolbar=no, menubar=no, scrollbars=no, resizable=yes");
